...
public class MyPushMessageReceiver extends FrontiaPushMessageReceiver {
    ...
    private void updateContent(Context context, String content) {
        ...
        if (content.contains("通知")) {
            if (content.contains("人才市场")) {
                Intent intent = new Intent();
                ContentValues contentValues = new ContentValues();
                contentValues.put("type", 1);
                ContentResolver resolver = ZhaopinzhApp.getInstance().getContentResolver();
                resolver.insert(MessageProvider.NEW_HINT_URI, contentValues);
                intent.setClass(context.getApplicationContext(), FairActivity.class);
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                context.getApplicationContext().startActivity(intent);
            } else if(content.contains("校园")){
                Intent intent = new Intent();
                ContentValues contentValues = new ContentValues();
                contentValues.put("type", 2);
                ContentResolver resolver = ZhaopinzhApp.getInstance().getContentResolver();
                resolver.insert(MessageProvider.NEW_HINT_URI, contentValues);
                intent.setClass(context.getApplicationContext(), NetJobConciseListActivity.class);
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                context.getApplicationContext().startActivity(intent);
            }
        }
