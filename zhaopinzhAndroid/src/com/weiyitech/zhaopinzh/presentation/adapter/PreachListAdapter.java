 ...
 */
public class PreachListAdapter extends BaseAdapter {
    public ArrayList<PreachmeetingConciseRsp> preachmeetingConciseRspArrayList;
    public CampusActivity activity;
    public boolean mBusy = false;

    public PreachListAdapter(Activity context, ArrayList<PreachmeetingConciseRsp> preachmeetingConciseRspArrayList) {
        super();
        ...
        holder.favoriteImgBtn.setOnClickListener(new ImageButton.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (favorImageButton.isSelected()) {
                    PreachListAdapter.this.preachmeetingConciseRspArrayList.get(position).isFavorite = false;
                    favorImageButton.setSelected(false);
                    ContentResolver resolver = activity.getContentResolver();
                    ContentValues contentValues = new ContentValues();
                    String mSelectionClause = "preach_meeting_id = CAST(? AS INT)";
                    String[] mSelectionArgs = {String.valueOf(PreachListAdapter.this.preachmeetingConciseRspArrayList.get(position).preachMeetingId)};
                    int val = resolver.delete(MessageProvider.FAVOR_PREACH_MEETING_URI, mSelectionClause, mSelectionArgs);
                } else {
                    PreachListAdapter.this.preachmeetingConciseRspArrayList.get(position).isFavorite = true;
                    favorImageButton.setSelected(true);
                    //存入数据库
                    ContentResolver resolver = activity.getContentResolver();
                    ContentValues contentValues = new ContentValues();
                    Common.putPreachMeeting(contentValues, PreachListAdapter.this.preachmeetingConciseRspArrayList.get(position));
